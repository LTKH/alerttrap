...
func (db *Client) LoadAlerts() ([]cache.Alert, error) {
    ...
    rows, err := db.client.Query("select * from alerts a where a.ends_at > $1 - 600 and a.ends_at = (select max(ends_at) from alerts where group_id = a.group_id)", time.Now().UTC().Unix())
    if err != nil {
        return result, err
    }
    defer rows.Close()
